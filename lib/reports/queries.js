...
// The last `days` IST date labels ending today, oldest first — for the
// Dashboard's leads-over-time line chart. Zero-filled per day (not just
// days that have a lead) so the line is continuous, not sparse dots.
function istRecentDateLabels(days) {
  const now = new Date();
  const labels = [];
  for (let i = days - 1; i >= 0; i--) {
    const shifted = new Date(now.getTime() + IST_OFFSET_MINUTES * 60000 - i * 24 * 60 * 60000);
    labels.push(shifted.toISOString().slice(0, 10));
  }
  return labels;
}
...
export async function getDashboardKpis(supabase) {
  ...
  const [leadsRes, quotationsRes, bookingsRes, followupsRes] = await Promise.all([
    supabase.from("leads").select("id, status, created_at, lead_sources(name)"),
  ...
  // Chart data: the same fields/rows already fetched above for the KPI cards,
  // just grouped for display — not a new metric or business rule. Quotation
  // outcomes intentionally overlap (an accepted quotation was also sent) since
  // each bar restates one of Sales Reports' already-defined, independent
  // counts, not a mutually-exclusive breakdown of one total.
  const leadsByDay = new Map();
  for (const l of leads) {
    const day = istDateLabel(l.created_at);
    leadsByDay.set(day, (leadsByDay.get(day) || 0) + 1);
  }

  const charts = {
    leadStatusChart: groupCount(leads, (l) => l.status),
    leadSourceChart: groupCount(leads, (l) => l.lead_sources?.name || "Unspecified"),
    quotationOutcomeChart: [
      { label: "Sent", count: sentQuotations.length },
      { label: "Accepted", count: acceptedQuotations.length },
      { label: "Rejected", count: quotations.filter((q) => q.status === "rejected").length },
    ],
    // Same istDateLabel bucketing as Lead Reports' "date-wise" breakdown
    // (master plan §12), just windowed to 14 days and zero-filled for a
    // continuous line instead of frequency-sorted bars.
    leadsOverTimeChart: istRecentDateLabels(14).map((day) => ({ label: day, count: leadsByDay.get(day) || 0 })),
  };
