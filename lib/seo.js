...
export function pageMetadata(pathname, overrides = {}) {
  ...
  // CMS seo_metadata.canonical_url is an absolute URL when set (see SeoForm's
  // placeholder); overrides.canonical lets a Phase 6 CMS-driven page use it
  // as-is instead of the default self-referencing pathname.
  const canonical = overrides.canonical || `${siteConfig.domain}${pathname === "/" ? "" : pathname}`;
  const imageUrl = overrides.image
    ? overrides.image.startsWith("http")
      ? overrides.image
      : `${siteConfig.domain}${overrides.image}`
    : `${siteConfig.domain}/logo.svg`;

  return {
    title,
    description,
    alternates: { canonical },
    // seo_metadata.indexable maps to this — a CMS author marking a page
    // "not indexable" must actually suppress indexing, not just hide a UI toggle.
